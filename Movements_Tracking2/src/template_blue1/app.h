...
{
	ASSERT_F (argc == 1, "%s", "No arg allowed");
	ASSERT_F (argv != NULL, "%s", "NULL error");
	app->flags = 0;
	{
	...
	app->w = APP_WINDOW_WIDTH;
	app->h = APP_WINDOW_HEIGHT;
...
void app_handle_input (struct Application * app)
{
	...
	switch (e.type)
	{
		...
		switch (app->event.window.event)
		{
			...
			app->w = (float)app->event.window.data1;
			app->h = (float)app->event.window.data2;
			TRACE_F ("%f", (double) app->w);
			TRACE_F ("%f", (double) app->h);
