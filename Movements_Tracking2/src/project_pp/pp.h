...
)
{
	...
	for (i = j + 1; i < n; ++ i)
	{
		...
		if (d3 > dmin [j]) {continue;}
		
		//Store new minumum distance.
		dmin [j] = d3;
		//TRACE_F ("%i <> %i %f", i, j, (double)dmin [j]);
		
		//if (next [i] != UINT32_MAX) {continue;}
