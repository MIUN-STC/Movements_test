...
#define APP_VALUES_MAX 100
#define APP_SHADER_FILES_XYUV "src/project_vidplot/xyuv.glvs;src/project_vidplot/xyuv.glfs"
#define APP_SHADER_XYUV 0
#define APP_UNIFORM_TRANSFORM 0
#define APP_UNIFORM_COLOR 1
#define APP_BUF_SIZE (10000*sizeof (float)*4)
...
void asset_init (struct Assets * as, uint32_t * flags)
{
	...
	{
		as->values1 = (float *) malloc (APP_VALUES_MAX * sizeof (float));
		as->values1_n = APP_VALUES_MAX;
		FILE * f = fopen ("data/values.txt", "r");
		ASSERT (f != NULL);
		read_values (f, 1, &as->values1_n, as->values1);
		fclose (f);
	}
	...
	mesh_add_plot (as->ma + APP_MESH_PLOT1, &vfirst, as->buf [0], as->values1_n, as->values1, GL_LINE_STRIP);
	mesh_add_plot (as->ma + APP_MESH_PLOTP1, &vfirst, as->buf [0], as->values1_n, as->values1, GL_POINTS);
