...
void enc_open (struct Encoder * enc)
{
	...
	else if (strcmp (enc->machine, "cube") == 0)
	{
		//x264 bitrate unit is kbps and not bps.
		enc->bitrate = enc->bitrate / 1000;
		snprintf 
		(
			buffer, 
			sizeof (buffer), 
	...
	ASSERT_F (buffer [0] != '\0', "%s", "No string for popen");
	fprintf (stderr, "%s\n", buffer);
