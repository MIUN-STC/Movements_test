...
#ifndef CAM_ASSERT_F
#define CAM_ASSERT_F(A, F, ...) if (A) {}
#endif

#ifndef CAM_TRACE_F
#define CAM_TRACE_F(F, ...)
#endif


#define CAM_LIST_MAX 100
#define CAM_BUFFER_MAX 1000
#define CAM_AVOID_SEGFAULT_SLEEP_SEC 5
...
int cam_get_str (struct Gev_Cam * cam, char const * name, int len, char * value)
{
	...
	CAM_TRACE_F ("%28s %40s %30s : %s (%i)", "GevGetFeatureValueAsString", name, value, get_error_name (status), status);
	return status;
}
...
int cam_get_u32 (struct Gev_Cam * cam, char const * name, uint32_t * value)
{
	...
	CAM_TRACE_F ("%28s %40s %30lu %i : %s (%i)", "GevGetFeatureValue", name, (long unsigned)(*value), type, get_error_name (status), status);
...
int cam_get_float (struct Gev_Cam * cam, char const * name, float * value)
{
	...
	CAM_TRACE_F ("%28s %40s %30f %i : %s (%i)", "GevGetFeatureValue", name, (float)(*value), type, get_error_name (status), status);
	if (status != GEVLIB_SUCCESS) {goto error;}
...
int cam_set_str (struct Gev_Cam * cam, char const * name, char const * value)
{
	...
	CAM_TRACE_F ("%28s %40s %30s : %s (%i)", "GevSetFeatureValueAsString", name, value, get_error_name (status), status);
	return status;
}
...
int cam_set_u32 (struct Gev_Cam * cam, char const * name, uint32_t value)
{
	...
	CAM_TRACE_F ("%28s %40s %30lu : %s (%i)", "GevSetFeatureValue", name, (long unsigned) value, get_error_name (status), status);
	return status;
}
...
int cam_set_float (struct Gev_Cam * cam, char const * name, float value)
{
	...
	CAM_TRACE_F ("%28s %40s %30f : %s (%i)", "GevSetFeatureValue", name, value, get_error_name (status), status);
	return status;
}
...
int cam_close (struct Gev_Cam * cam)
{
	...
	Gev_Status status;
	cam_stop (cam);
	CAM_TRACE_F ("@ Trying to avoid segmentation fault by sleeping for %i sec before freeing image queue...", CAM_AVOID_SEGFAULT_SLEEP_SEC);
	sleep (CAM_AVOID_SEGFAULT_SLEEP_SEC);
...
int cam_setup (struct Gev_Cam * cam)
{
	Gev_Status status;
	...
	if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "ExposureAuto", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessMode", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessAlgoMinTimeActivation", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "exposureAutoMinValue", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "exposureAutoMaxValue", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessTargetSource", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessTarget", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	//status = cam_get_str (cam, "gainAutoSource", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "gainAutoMaxValue", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "gainAutoMinValue", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessAlgoSource", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "autoBrightnessSequence", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "outputLineSource", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "LineMode", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "LineStatusAll", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
	status = cam_get_str (cam, "LineSelector", sizeof (buf), buf);
	//if (status != GEVLIB_SUCCESS) {goto error;}
...
int cam_open (struct Gev_Cam * cam)
{
	...
	status = GevGetCameraList (list, CAM_LIST_MAX, &count);
	if (status != GEVLIB_SUCCESS) {goto error;}
	CAM_TRACE_F ("Camera count: %i", (int) count);
	for (int i = 0; i < count; i = i + 1)
	{
		struct Gev_Camera * c = list + i;
		CAM_ASSERT_F (c != NULL, "%s", "");
...
int cam_open_serial (struct Gev_Cam * cam, char const * sn)
{
	if (sn == NULL) {return cam_open (cam);}
	
	Gev_Status status;
	status = GevApiInitialize ();
	CAM_ASSERT_F (status == GEVLIB_SUCCESS, "%s", "");
	if (status != GEVLIB_SUCCESS) {goto error;}
	status = GevOpenCameraBySN ((char *)sn, GevExclusiveMode, &(cam->handle));
	CAM_TRACE_F ("%s %s : %s (%i)", "GevOpenCameraBySN", sn, get_error_name (status), status);
	if (status != GEVLIB_SUCCESS) {goto error;}
	CAM_ASSERT_F (cam->handle != NULL, "GevOpenCamera%s", "");
	errno = 0; //Reset errno because errno can be set inside GevOpenCamera.
	cam_setup (cam);
...
int cam_print_transfer_status (struct Gev_Cam * cam)
{
	...
	uint32_t buf_used = 0;
	uint32_t buf_free = 0;
	uint32_t buf_trashed = 0;
	enum GevBufferCyclingMode mode;
	Gev_Status status;
	status = GevQueryImageTransferStatus (cam->handle, &buf_total, &buf_used, &buf_free, &buf_trashed, &mode);
	if (status != GEVLIB_SUCCESS) {return status;}
	//CAM_TRACE_F ("%s : %s (%i)", "GevReleaseImage", get_error_name (status), status);
	fprintf 
	(
		stdout, 
		"total: %06lu, used: %06lu, free: %06lu, trashed: %06lu ", 
		(lu)buf_total,
		(lu)buf_used,
		(lu)buf_free,
		(lu)buf_trashed
