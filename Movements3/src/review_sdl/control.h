...
void ctrl_auto_exposure2 (struct Control * ctrl, struct Gev_Image * image, struct Gev_Cam * cam, uint8_t target)
{
	float d = 0;
	...
	//printf ("d %f\n", d);
	ctrl->exposure += (d * ctrl->exposure);
	ctrl->exposure = MAX (26, ctrl->exposure);
	ctrl->exposure = MIN (1000000, ctrl->exposure);
